        ...
        if lenone + lentwo > grammarListLengthMaximum:
            if lenone:
                if lenone < grammarListLengthMaximum:
                    ...
                    print('appendList: can only append %s items (out of %s) to grammar list: "%s"' %
                          (newAllowed, lentwo, listName))
            ...
            if lenone:
                print('appendList, warning list becoming large: appending %s items to grammar list: "%s", new length: %s' %
                      (lentwo, listName, lenone + lentwo))
            else:
...
class IniGrammar(IniGrammarAncestor):
        ...
        try:
            ...
            if self.gramSpec:
                if isinstance(self.gramSpec, list):
                    ...
                    raise TypeError(
                        f'IniGrammar "{self.name}", gramSpec should be string or list, not: "{type(self.gramSpec)}"') from exc
    ...
    def checkName(self):
        ...
        try:
            ...
            if self.name is not self.__class__.name:
                return self.name
        try:
...
#             return
    ...
    def showInifile(self, body=None, grammarLists=None, ini=None,
        ...
        grammarwordsLists = [ll for ll in grammarLists if ll.startswith('grammar ')]
        grammarLists = [ll for ll in grammarLists if not ll.startswith('grammar ')]
    ...
    def fillGrammarLists(self, listOfLists=None):
        ...
        if not self.ini:
            print(
                f'--- no valid ini file for grammar: "{self.GetName()}", will not fill grammar lists\n\tPlease try to correct via "edit {self.getName()}"')
            return
        ini = self.ini
        fromGrammar = copy.copy(self.validLists)
        allListsFromIni = ini.get()

        nonemptyListsFromIni = [ll for ll in allListsFromIni if ini.get(ll)]  # empty sections ignored
        if self.iniIgnoreGrammarLists:
            self.removeFromList(fromGrammar, self.iniIgnoreGrammarLists)
        if listOfLists:
            for ll in listOfLists:
                if ll not in fromGrammar:
                    self.errorf('fillGrammarLists, list name not in grammar: %s' % ll)
                    continue
                if ll not in allListsFromIni:
                    self.errorf('fillGrammarLists, list name not in ini file: %s' % ll)
                    continue
                if self.fillList(ll):
                    fromGrammar.remove(ll)

        else:
            for ll in nonemptyListsFromIni:
                if ll not in fromGrammar:
                    continue
                if self.fillList(ll):
                    fromGrammar.remove(ll)
        ...
        if fromGrammar:
            changes = 0
            for ll in fromGrammar[:]:
                if ll in self.iniIgnoreGrammarLists:
                    continue
                if self.fillList(ll):
                    fromGrammar.remove(ll)
                elif ll in allListsFromIni:
                    pass
                    # print('\t%s: warning, empty section for list "%s" in inifile'% (self.name, ll))
                else:
                    changes = 1
                    self.emptyList(ll)
                    self.ini.set(ll)
    ...
    def fillList(self, listName):
        ...
        ll = self.ini.get(n)
        if ll:
            # print '%s: filling list %s with %s items'% (self.name, listName, len(ll))
            self.setList(n, ll)
            return 1
        ...
        if newDate > self.iniFileDate:
            ...
            try:
                ...
                if isinstance(self.gramSpec, str):
                    ...
                    raise TypeError('At reload inifile of IniGrammar "%s", gramSpec should be a string or a list, not: %s' %
                                    (self.name, type(self.gramSpec)))
    ...
    def makeDefaultInifile(self, inifile=None, enxVersion=None):
        ...
        if enxVersion:
            ...
            self.ini.set('an instruction', 'intro',
                         'The english inifile is taken as example for this new language inifile for "%s"' % self.language)
            ...
            self.ini.set('an instruction', 'note 3',
                         'Translate words in other sections as appropriate, see also Unimacro website...')
            self.ini.set('an instruction', 'note 4', 'If this is a new language for existing grammar files,')
            self.ini.set('an instruction', 'note 5',
                         'please contact the Unimacro developers to publish your translation in future versions!')
            ...
            if self.language == 'enx':
                self.ini.set('an instruction', 'note 2',
                             'If this is a grammar useful to others, please consider publishing it in the Unimacro area,')
                self.ini.set('an instruction', 'note 3',
                             'you can contact the Unimacro developers through the Unimacro website for this.')
                self.ini.set('an instruction', 'note 4', 'This section can be deleted after reading')
            else:
                self.ini.set('an instruction', 'note 2',
                             'Please consider to write your grammar in English first and then make a translation to "%s"' % self.language)
    ...
    def fillDefaultInifile(self, ini):
        ...
        if hasattr(self, "validLists") and hasattr(self, 'iniIgnoreGrammarLists'):
            for ll in self.validLists:
                if ll not in self.iniIgnoreGrammarLists:
                    ini.set(ll)
        ini.write()
        ...
        if istop:
            if actions.topWindowBehavesLikeChild(progInfo):
                ...
                if actions.childWindowBehavesLikeTop(progInfo):
                    self.errorf(
                        'getTopOrChild: top mode, altough of class "%s", but because of "child behaves like top" in "actions.ini"' % childClass)
                    istop = True
                else:
...
#             return '\n'.join([t.strip() for t in [''.join(ll) for ll in lines]])
#     else:
