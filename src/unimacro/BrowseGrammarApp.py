...
class GrammarDialog(dialog.Dialog):
    ...
    def SetRuleItems(self, Rule, InnerRules):
        ...
        Items = []
        if not (IsGrammar or IsSimple):
            Items.append(('<Syntax>', FoldedContents, Rule))
...
class TrainGrammarDialog(GrammarDialog):
    ...
    def GetTextChunks(self):
        ...
        if ToTrain[0] == '<imported>':
            ToTrain.remove('<imported>')
        return ToTrain
...
def CreateBrowseDialog():
    ...
    Grammars.Sort()
    if Exclusive:
        Name = 'Exclusive Grammars (Active Rules)'
    elif All:
        Name = 'All Grammars'
    else:
        Name = 'Active Grammars'
